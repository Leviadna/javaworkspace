...
public class JoinForm extends JFrame implements ActionListener{
	...
	public JoinForm() {
		...
		la_title.setFont( new Font("a솜사탕", Font.BOLD, 24));
	...
	public void checkValue() {
		...
		//String str = t_pass.getText(); //deprecated 되었으므로 사용하지 말아야 한다
		//System.out.println("당신이 입력한 비밀번호는 "+str); //잘 실행되지만 사용하기엔 애매
		//해결책?
		//char[] pass = t_pass.getPassword();
		//char 배열로 데이터 받고 원하는 형태인 String으로 다시 변환
		...
		if(t_id.getText().length()==0) {
			...
			JOptionPane.showMessageDialog(this, "비밀번호는 8자 이상 입력하세요");
			t_pass.requestFocus();
		}else if(count<1){
			JOptionPane.showMessageDialog(this, "1개 이상의 취미를 선택해주세요");
		}else {
			//오타클 DB입력
		}
	}
