...
public class loginform {
	public static void main(String[] args) {
		//Frame frame; //null조차 자동으로 초기화되지 않는다 (지역변수라서)
		//결론: 지역변수는 초기화하는 습관을 갖자
		Frame frame = new Frame("로그인 폼 타입 1");
		/*
		Frame의 BorderLayout의 한 방위(동, 서, 남, 북, 센터)는
		컴포넌트들이 공존할 수 없고 크기 역시 컨테이너까지 늘어나버린다
		(ex: 대왕 버튼, 대왕 텍스트 필드 등)
		해결을 위해서 여러 컴포넌트를 올려놓고 공존시킬 수 있는 컨테이너를 이용한다
		컨테이너중 가장 많이 활용되는 컴포넌트인 Panel을 이용해보자
		*/
		Panel p_center=new Panel(); //패널은 디폴트, 레이아웃은 FlowLayout
		
		//패널에 적용된 배치 방법을 GridLayout으로 바꾸기
		...
		Panel p_south=new Panel(); //남쪽에 버튼을 얹힐 패널
		Button bt_login=new Button("Login");
		Button bt_join=new Button("Join");
		
		//라벨의 경계를 알기 위해 배경색을 적용
		la_id.setBackground(Color.yellow);
		la_pass.setBackground(Color.green);
		
		p_center.add(la_id); //센터에 라벨 부착
		p_center.add(t_id); //방위를 명시하지 않을 경우 디폴트는 센터
